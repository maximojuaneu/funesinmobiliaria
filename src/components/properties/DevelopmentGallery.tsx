...
export default function DevelopmentGallery({ photos, title }: Props) {
  ...
  // Build rows: groups of [1 full-width, 2 side-by-side]
    ...
    <>
        ...
        {photos.map((photo, j) => (
          ...
          >
            <Image
              ...
              sizes="16vw"
            />
            {j === photos.length - 1 && photos.length > 1 && (
      ...
      {open && (
        <div className={`fixed inset-0 z-[1100] bg-black flex flex-col${lightboxIdx !== null ? ' invisible' : ''}`}>
          ...
          <div className="flex items-center px-6 py-4 flex-shrink-0 border-b border-white/10">
